"""Lector de tablas Visual FoxPro (.DBF) — sin dependencias externas.

Copia del lector del ETL de Créditos (`modules/creditos/backend/app/etl/dbf.py`): los módulos de
Portezuelo no comparten código. Acá se usa para importar el padrón de clientes del sistema anterior.

Soporta los tipos usados por CCyPP: C (char), N (numérico), F (float), I (entero 4 bytes), D (fecha),
T (datetime), L (lógico), Y (currency), B (double), M (memo).

Lee en streaming, que es lo que hace falta acá (maeclientes.dbf son ~78.000 registros y 49 MB):
