...
def _to_response(cred: InterbankingCredential) -> dict:
    return {
        ...
        "consolidation_account_number": cred.consolidation_account_number,
        "consolidation_account_type": cred.consolidation_account_type,
        "consolidation_bank_number": cred.consolidation_bank_number,
        "consolidation_currency": cred.consolidation_currency,
        "payment_account_number": cred.payment_account_number,
        "payment_account_type": cred.payment_account_type,
        "payment_bank_number": cred.payment_bank_number,
        "payment_currency": cred.payment_currency,
...
def save_config(data: CredentialCreate, db: Session = Depends(get_db)):
    ...
    cred = InterbankingCredential(
        ...
        consolidation_account_number=data.consolidation_account_number or None,
        consolidation_account_type=data.consolidation_account_type or None,
        consolidation_bank_number=data.consolidation_bank_number or None,
        consolidation_currency=data.consolidation_currency or None,
        payment_account_number=data.payment_account_number or None,
        payment_account_type=data.payment_account_type or None,
        payment_bank_number=data.payment_bank_number or None,
        payment_currency=data.payment_currency or None,
        is_active=True,
    )
    db.add(cred)
    db.commit()
